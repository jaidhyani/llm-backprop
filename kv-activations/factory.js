...
const SEQUENCES = {
  sec1: ['The', 'cat', 'sat'],
  sec2: ['The', 'cat', 'sat', 'on'],
  sec3: ['The', 'cat', 'sat', 'on', 'the', 'mat'],
  sec4: ['The', 'cat', 'sat', 'on', 'the', 'mat'],
};
...
function drawPredBar(ctx, cx, cy, text, a, glow, fontSize) {
  ...
  ctx.fillStyle = rgba(COL.nodeFill, a * 0.6);
  rRect(ctx, x, y, tw, th, th/2); ctx.fill();
  ctx.save();
  ctx.setLineDash([3, 3]);
  ctx.strokeStyle = rgba(color, a * (0.3 + glow * 0.4));
...
function drawSec1(ctx, W, H, t) {
  ...
  const s = W / 1100;
  const cx = W / 2;
  const lSz = Math.round(32 * s * density);
  ...
  const embSz = Math.round(6 * s * density);
  const resW = Math.round(24 * s * density);
  const resH = Math.round(5 * s * density);
  ...
  const pillFs = Math.round(14 * s * density);
  const predFs = Math.round(12 * s * density);
  const pillGap = 4 * s;
  ...
  for (let r = 0; r < resPositions.length; r++) {
    ...
    const resLabel = r < L - 1
      ? `Layer ${r+1} Residual Activations`
      : 'Unembedding Residual';
...
function drawSec2(ctx, W, H, t) {
  ...
  drawBg(ctx, W, H);
  const s = W / 1100;
  const lSz = Math.round(28 * s * density);
  ...
  const resW = Math.round(22 * s * density);
  const resH = Math.round(5 * s * density);
  const embSz = Math.round(5 * s * density);
  ...
  const fs = Math.round(14 * s * density);
  const predFs = Math.round(12 * s * density);
  ...
  for (let pos = 0; pos <= drawCycle; pos++) {
    ...
    for (let r = 0; r < resPs.length; r++) {
      ...
      const resLabel = r < L - 1 ? `Layer ${r+1} Residual Activations` : 'Unembedding Residual';
  ...
  // Epilogue: final predicted token flies up
  if (cycle >= N) {
...
function drawSec3(ctx, W, H, t) {
  ...
  const s = W / 1100;
  const lSz = Math.round(28 * s * density);
  const kvSz = Math.round(14 * s * density);
  const resW = Math.round(22 * s * density);
  const resH = Math.round(5 * s * density);
  ...
  for (let pos = 0; pos <= drawCycle; pos++) {
    ...
    for (let r = 0; r < resPs.length; r++) {
      ...
      let resLabel;
      if (r === 0) resLabel = 'Embedding Residual';
      else if (r <= L - 1) resLabel = `Layer ${r} Residual Activations`;
      else resLabel = 'Unembedding Residual';
      ctx._hitTargets.push({ x: cx - resW/2, y: resPs[r] - resH/2, w: resW, h: resH, label: resLabel });
    }
...
function drawSec4(ctx, W, H, t) {
  ...
  const tokFs = Math.round(14 * s * density);
  const predFs = Math.round(12 * s * density);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  createSection(document.getElementById('sec4'), drawSec4,
    sec4GradStart + (n4 + L - 2) * sec4WaveSpeed + 2.0);
});
